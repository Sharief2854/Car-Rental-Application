...
// import { userSericeObj } from '../../admin-service/adminService';
...
function UserProfile()
{
    ...
    function getData()
    {
        userSericeObj.getData({email:localStorage.getItem('email')}).then((data)=>
        {
            if(data.data)
            {
    ...
    function adminEditField()
    {
        function editBtn()
        {
            ...
            obj.email=email;
            userSericeObj.updateProfile(obj).then((data)=>
            {
                if(data.data)
                {
                    alert("updated");
